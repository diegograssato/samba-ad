...
if [ ! -f /etc/dhcpd/dhcp-dns.keytab ]; then
    echo "Required keytab /etc/dhcpduser.keytab not found, it needs to be created."
    echo "Use the following commands as root"
    echo "samba-tool domain exportkeytab --principal=${SETPRINCIPAL} /etc/dhcpd/dhcpd.keytab"
...
if [ "$?" != "0" ]; then
    ...
    kinit -F -k -t /etc/dhcpd/dhcp-dns.keytab -c /tmp/dhcp-dyndns.cc "${SETPRINCIPAL}"
    if [ "$?" != "0" ]; then
